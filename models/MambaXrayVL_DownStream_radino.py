...
class MambaXrayVLDownStreamradino(pl.LightningModule):
    """
    ...
    def __init__(self, args, freeze_radino=False):
        ...
        # 2. 投影层
        llama_hidden = args.llama_hidden if hasattr(args, "llama_hidden") else 4096
        self.dino_proj = nn.Linear(768, llama_hidden)
        # 冻结rad-dino参数
        if freeze_radino:
    ...
    def encode_img(self, images, segmentation=None):
        ...
        for image in images:
            device = image.device
            dino_feat = self.rad_dino(image).last_hidden_state  # [N, 768] 或 [B, N, 768]
            if dino_feat.dim() == 2:
                dino_feat = dino_feat.unsqueeze(0)  # [1, N, 768]
            dino_feat_proj = self.dino_proj(dino_feat)  # [B, N, llama_hidden]
            image_embeds.append(dino_feat_proj)
        image_embeds = torch.stack(image_embeds).mean(0)
