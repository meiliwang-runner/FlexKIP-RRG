...
class MambaXrayVLDownStream(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        print(self.rad_dino.config.hidden_size)
        self.fusion_module = FeatureFusion(mamba_dim=1024, rad_dino_dim=768)
        # 特征融合模块--添加
        
        # 添加线性层，将视觉特征映射到 LLaMA 的输入维度
        self.llama_proj = nn.Linear(1024, self.llama_model.config.hidden_size)  # 假设视觉特征维度为 1024
        # 添加
    ...
    def encode_img(self, images):
        image_embeds = []
        mamba_feats = []  # 保存 mamba_feat
        rad_dino_feats = []  # 保存 rad_dino_feat
        ...
        for image in images:
            ...
            # 融合
            fused_feat = self.fusion_module(mamba_map, dino_map)  # [B, 1024, 14, 14]

            # 保存特征
            mamba_feats.append(mamba_feat)
            rad_dino_feats.append(dino_feat)

            # 后处理
            image_embeds.append(fused_feat)

        # 对 batch 的特征进行平均
        image_embeds = torch.stack(image_embeds).mean(0)  # [B, 1024, 14, 14]

        # 空间平均 (H, W)
        image_embeds = image_embeds.mean(dim=[2, 3])  # [B, 1024]

        # 投影到大语言模型的输入维度
        inputs_llama = self.llama_proj(image_embeds)  # [B, llama_input_dim]
