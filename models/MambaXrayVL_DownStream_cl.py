...
class MambaXrayVLDownStream_cl(pl.LightningModule):
    """
    ...
    def __init__(self, args):
        ...
        self.contrastive_loss_weight = args.contrastive_loss_weight  # 添加对比损失权重
    ...
    def forward(self, samples):
        ...
        ce_loss = outputs.loss # 交叉熵损失
        
         # 对比学习部分
        # 提取图像和文本特征
        image_features = img_embeds.mean(dim=1)  # 图像特征
        text_features = self.embed_tokens(to_regress_tokens.input_ids).mean(dim=1)  # 文本特征
        ...
        # 合并损失
        total_loss = ce_loss + self.contrastive_loss_weight * contrastive_loss

        return {"loss": total_loss, "ce_loss": ce_loss, "contrastive_loss": contrastive_loss}
