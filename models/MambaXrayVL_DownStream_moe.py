...
class MambaXrayVLDownStreammoe(pl.LightningModule):
    """
    ...
    #     save_to = os.path.join(
    #         self.hparams.savedmodel_path, 'checkpoints',
    #         "checkpoint_epoch{}_step{}_bleu{:3f}_cider{:3f}_{}.pth".format(
    #             current_epoch, global_step, eval_res['Bleu_4'], eval_res['CIDEr'], current_time
    #         ),
    #     )
    #     self.print("Saving checkpoint at step {} to {}.".format(global_step, save_to))
    #     torch.save(save_obj, save_to)
    ...
    #     # 添加时间戳到 result 文件夹
    #     current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    #     result_folder = os.path.join(self.hparams.savedmodel_path, f'result_{current_time}')
    #     os.makedirs(result_folder, exist_ok=True)
    #     current_epoch, global_step = self.trainer.current_epoch, self.trainer.global_step
    #     json.dump(hypo, open(os.path.join(result_folder, f"result_{current_epoch}_{global_step}.json"), 'w', encoding='utf-8'), ensure_ascii=False)
    #     json.dump(ref, open(os.path.join(result_folder, 'refs.json'), 'w', encoding='utf-8'), ensure_ascii=False)
    ...
    #     # 添加时间戳到 test result 文件夹
    #     current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    #     result_folder = os.path.join(self.hparams.savedmodel_path, f'result_{current_time}')
    #     os.makedirs(result_folder, exist_ok=True)
    #     json.dump(hypo, open(os.path.join(result_folder, f"test_result_{current_time}.json"), 'w', encoding='utf-8'), ensure_ascii=False)
    #     json.dump(ref, open(os.path.join(result_folder, 'test_refs.json'), 'w', encoding='utf-8'), ensure_ascii=False)
    #     self.print(f"Test result of {self.hparams.delta_file}: {eval_res}")
